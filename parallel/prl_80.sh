...
#SBATCH --job-name=shellSort80
#SBATCH --output=shellSort80.out
#SBATCH --error=shellSortError80.err
#SBATCH --time=0-15:0
...
g++ -g -Wall -fopenmp -std=c++0x shellsort_prl.cpp -o shell
	...
	do
			for size in 80000000 #tamanho do problema
			do   	
				echo -e "\n$cores\t$size\t\t" >> "parallel_80_core.txt" 

				for tentativa in $(seq $tentativas) #Cria uma vetor de 1 a "tentativas"
				do
					./shell $cores $size >> "parallel_80_time.txt"
				done
			done
